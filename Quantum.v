...
Program Fixpoint swap_to_0' (n i : nat) {pf : lt (i+1) n} {struct i}: Matrix (2^n) (2^n) := 
  match i with
  | O => swap ⊗ Id (2^(n-2))
  | S i' =>  (Id (2^i'-1) ⊗ swap ⊗ Id (2^(n-i'-2))) × (* swap i-1 with i *)
            swap_to_0' n i' × 
            (Id (2^(i'-1)) ⊗ swap ⊗ Id (2^(n-i'-2))) (* swap i-1 with 0 *)
  end.
...
Program Fixpoint move_to_0' (n i : nat) {pf : lt (i+1) n} {struct i}: Matrix (2^n) (2^n) := 
  match i with
  | O => swap ⊗ Id (2^(n-2))
  | S i' =>  (Id (2^i'-1) ⊗ swap ⊗ Id (2^(n-i'-2))) × swap_to_0' n i
  end.
...
(*
Definition new0_op {n} : Superoperator n (2*n) := fun ρ => |0⟩⟨0| ⊗ ρ.
Definition new1_op {n} : Superoperator n (2*n) := fun ρ => |1⟩⟨1| ⊗ ρ. 
*)

Definition new0_op {n} : Superoperator n (2*n) := super |0⟩.
Definition new1_op {n} : Superoperator n (2*n) := super |1⟩.
Definition meas_op {n} : Superoperator n n := fun ρ => super |0⟩⟨0| ρ .+ super |1⟩⟨1| ρ.
Definition discard_op {n} : Superoperator (S n) n := fun ρ => super ⟨0| ρ .+ super ⟨1| ρ.
...
Lemma pure_unitary : forall {n} (U ρ : Matrix n n), 
  WF_Matrix ρ -> unitary_matrix U -> Pure_State ρ -> Pure_State (super U ρ).
Proof.
  intros n U ρ WF H P.
...
Proof. apply pure_unitary. 
       + unfold WF_Matrix. intros x y [H | H]; mlra; omega.
       + apply H_unitary.
       + apply pure1.
Qed.
...
Lemma mixed_unitary : forall {n} (U ρ : Matrix n n), 
  WF_Matrix ρ -> unitary_matrix U -> Mixed_State ρ -> Mixed_State (super U ρ).
Proof.
  intros n U ρ WF H M.
  generalize dependent WF.
  induction M; intros WF.
  + apply Pure_S.
  ...
  + unfold unitary_matrix, super in *.
    ...
    apply Mix_S; trivial.
    apply IHM1. 
    (* ... and there's your compelling argument for 
           "all pure states must be well_formed". *)
Admitted.
