...
Ltac reification_wrt :=
  ...
  | [ |- ⟨?ls1⟩ ∘ ?ev = ⟨?ls2⟩ ] =>
    ...
    repeat rewrite split_list; try (apply f_equal2; [|simpl; reflexivity]); auto
  end.

(* The initial 'try reflexivity' solves the one-evar case *)
Ltac monoid := try reflexivity; repos_evar; prep_reification; strip_Some; reification_wrt; solve_permutation.
...
Example PCM_evar : forall a b c, exists d, a ∘ b ∘ c = c ∘ d ∘ a ∘ ⊤.   
Proof.
  intros. 
  eexists.
  monoid.
...
Example PCM_evar2 : forall a b c d e, exists x, a ∘ x ∘ c = b ∘ c ∘ d ∘ ⊤ ∘ e ∘ a. 
Proof.
  intros. 
  eexists.
  monoid.
...
Example PCM_evar3 : forall x y, exists w, x ∘ y = w.
Proof. 
  intros. 
  eexists.
  monoid.
