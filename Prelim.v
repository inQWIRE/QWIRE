...
Lemma double_mult : forall (n : nat), n + n = 2 * n. Proof. intros. lia. Qed.
...
Ltac unify_pows_two :=
  ...
  | [ |- context[ 4 ]]                  => replace 4 with (2^2) by reflexivity
  | [ |- context[ 0 + ?a]]            => rewrite plus_0_l 
  | [ |- context[ ?a + 0]]            => rewrite plus_0_r 
  | [ |- context[ 1 * ?a]]            => rewrite Nat.mul_1_l 
  | [ |- context[ ?a * 1]]            => rewrite Nat.mul_1_r 
  (* new *)
  | [ |- context[ ?a + 1]]            => rewrite Nat.add_1_r 
  (* /new *)
  | [ |- context[ 2 * 2^?x]]          => rewrite <- Nat.pow_succ_r'
  | [ |- context[ 2^?x * 2]]          => rewrite pow_two_succ_l
  | [ |- context[ 2^?x + 2^?x]]       => rewrite double_pow 
  | [ |- context[ 2^?x * 2^?y]]       => rewrite <- Nat.pow_add_r 
  | [ |- context[ ?a + (?b + ?c) ]]   => rewrite plus_assoc 
  | [ |- 2^?x = 2^?y ]                => apply pow_components; try lia 
  end.
